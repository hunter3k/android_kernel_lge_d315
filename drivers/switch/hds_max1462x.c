...
//                                                           
	...
	bool maxim_enabled = false;	
	static struct gpiomux_setting headset_active_cfg_gpio_detect ={
	...
	static struct gpiomux_setting headset_active_cfg_gpio_key ={
		.func = GPIOMUX_FUNC_GPIO,
		.drv = GPIOMUX_DRV_2MA,
		.pull = GPIOMUX_PULL_UP,
		.dir  = GPIOMUX_IN,
	...
	static struct msm_gpiomux_config headset_configs[] ={
		{
			.gpio = 0,
			.settings = {
				[GPIOMUX_ACTIVE]	= &headset_active_cfg_gpio_detect,
			},
		},
	
		{
			.gpio = 0,
			.settings = {
				[GPIOMUX_ACTIVE]	= &headset_active_cfg_gpio_key,
			},
		},
	};
...
static void button_pressed(struct work_struct *work)
{
	...
	rc = qpnp_vadc_read(hi->vadc_dev, P_MUX6_1_1,&result);
#endif
#ifdef CONFIG_ARCH_MSM8610
	rc = qpnp_vadc_read(hi->vadc_dev, P_MUX2_1_1,&result);
#endif
...
#if 0 //original
	msleep(40);   /* If you reduce the delay time, it will cause problems. */
#endif
	msleep(500);
...
static irqreturn_t button_irq_handler(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&ear_hook_wake_lock, 2 * HZ);
	state = atomic_read(&hi->is_3_pole_or_not);
	HSD_DBG("button_irq_handler: state:%d\n",state);
...
static int lge_hsd_probe(struct platform_device *pdev)
{
	...
	headset_configs[0].gpio = hi->gpio_detect;
	headset_configs[1].gpio = hi->gpio_key;		
	...
	hi->vadc_dev = qpnp_get_vadc(dev, "micen");
	if (IS_ERR(hi->vadc_dev)) {
		ret = PTR_ERR(hi->vadc_dev);
		if (ret != -EPROBE_DEFER)
			pr_err("missing vadc property, ret=%d\n", ret);
...
static int __init lge_hsd_init(void)
{
	...
	maxim_detic_node = of_find_compatible_node_with_rev_lge(NULL, NULL, "maxim,max1462x");
	
	if (maxim_detic_node) {
			maxim_enabled = true;
	} else {
		pr_err("%s: Failed to find maxim,max1462x node\n", __func__);
	}
	pr_info("%s: HSD.max1462x maxim_enabled: %d\n", __func__, maxim_enabled);
	
	if(!maxim_enabled) {
		HSD_DBG("maxim driver is NOT necessary!\n");
	    return 0;
	}
