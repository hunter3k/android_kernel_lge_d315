...
int ist30xx_calib_wait(void)
{
	...
	while (cnt-- > 0) {
		...
		if (ts_data->status.calib_msg) {
			tsp_info("Calibration Status : %d, Max raw gap : %d - (raw: %08x)\n",
...
//	ist30xx_reset();
	...
	if (ret) {
		...
		tsp_warn("Error CheckSum: 0x%08x(0x%08x)\n",
...
int ist30xx_auto_fw_update(struct ist30xx_data *data)
{
	...
	tsp_info("F/W version is new. 0x%08x -> 0x%08x\n",
...
int ist30xx_auto_param_update(struct ist30xx_data *data)
{
	...
	tsp_info("Param version is new. 0x%04x -> 0x%04x\n",
		 data->param_ver, val);
...
int ist30xx_check_auto_update(struct ist30xx_data *data)
{
	...
	if ((data->param_ver > 0) && (data->param_ver < 0xFFFF) &&
	    (data->fw.ver & MASK_FW_VER) == IST30XX_FW_VER3) { // FW core over 3.0
