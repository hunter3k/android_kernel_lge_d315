...
void __init msm8610_init_gpiomux(void)
{
    ...
    if (revision == HW_REV_C) { /*for Melfas Touch IC : enable LDO pin 82*/
    ...
    else if(revision == HW_REV_B)  /*for Melfas Touch IC : enable LDO pin 62*/
    {
        printk(KERN_INFO"[%s][TOUCH] HW_Revision = %d ", __func__, revision );
		msm_gpiomux_install(mms100s_ts_configs, ARRAY_SIZE(mms100s_ts_configs));
	}
	else if(revision >= HW_REV_D)
	{
	...
	} else { /*for LGD Incell Touch IC : Rev.A*/
	...
	if(revision == HW_REV_B) {
		msm_gpiomux_install(sd_card_det_rev_b, ARRAY_SIZE(sd_card_det_rev_b));
	} else {
